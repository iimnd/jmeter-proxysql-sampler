package com.iim.jmeter.proxysql;
 ...
 * Aug 27, 2019
 */
public class QueryDigestResetSampler extends AbstractJavaSamplerClient  {
	...
	public void setupTest(JavaSamplerContext context) {
		Logger logger   = Logger.getLogger( QueryDigestResetSampler.class.getName()); 
	...
	public SampleResult runTest(JavaSamplerContext context) {
		...
		if(this.databaseName == "") {
			 query = "select *from stats_mysql_query_digest_reset order by "+ this.order_by + " limit " + this.limit;
		}else {
			
			 query = "select *from stats_mysql_query_digest_reset where schemaname='"+this.databaseName+"' order by "+ this.order_by + " limit " + this.limit;
		}
